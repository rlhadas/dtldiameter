...
# These two variables are the strings we use to deliminate the two node types. Safer characters would be '(', ')', and
...
def event_node_to_string(event):
    ...
    """
    return event[0] + event_deliminator + mapping_node_to_string(event[1]) + event_deliminator + mapping_node_to_string(event[2])
...
def save_recon_graph(recon_graph, filepath):
    ...
    :param filepath:    The location we are saving this sif file.
    """
...
def load_recon_graph(filepath):
    ...
    :param filepath:    The location of the graph we are trying to load.
